...
class Trainer:
    ...
    def _log_to_mlflow(
        ...
        mlflow.catboost.log_model(
            model,
            "CatBoostClassifier",
            signature=infer_signature(x_train, model.predict(x_train)),  # type: ignore
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="데이터 기준일 (YYYY-MM-DD 형식)",
        dest="base_dt"
    )
    args = parser.parse_args()
    mlflow.set_tracking_uri("file:/home/codespace/mlruns") # 경로 설정 추가
